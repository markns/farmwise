...
# from farmbase_client.api.gaez import (
#     gaez_aez_classification,
#     gaez_growing_period,
#     gaez_suitability_index,
# )
    ...
    async with IsdaClient(base_url=ISDA_URL) as client:
        token = await login_login_post.asyncio(
            ...
            body=BodyLoginLoginPost(
                username=config.FARMBETTER_ISDA_USERNAME, password=config.FARMBETTER_ISDA_PASSWORD.get_secret_value()
            ),
        )
