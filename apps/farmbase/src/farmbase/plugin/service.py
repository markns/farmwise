...
# from farmbase.plugins.bases import OncallPlugin
...
def get_instance(*, db_session: Session, plugin_instance_id: int) -> Optional[PluginInstance]:
    ...
    return db_session.query(PluginInstance).filter(PluginInstance.id == plugin_instance_id).one_or_none()


def get_active_instance(*, db_session: Session, plugin_type: str, project_id=None) -> Optional[PluginInstance]:
...
def get_active_instances(*, db_session: Session, plugin_type: str, project_id=None) -> Optional[PluginInstance]:
    """Fetches the current active plugin for the given type."""
    return (
        db_session.query(PluginInstance)
        .join(Plugin)
...
def create_instance(*, db_session: Session, plugin_instance_in: PluginInstanceCreate) -> PluginInstance:
    ...
    project = project_service.get_by_name_or_raise(db_session=db_session, project_in=plugin_instance_in.project)
