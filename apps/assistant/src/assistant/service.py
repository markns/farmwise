...
from assistant.agent import ONBOARDING_AGENT, get_agents
from assistant.context import UserContext
from assistant.memory.session import SessionState, clear_session_state, get_or_create_session, set_session_state
from assistant.memory.zep import add_messages, get_memory
from assistant.schema import ResponseEvent, TextResponse, UserInput
from assistant.stream import _batch_stream_events
from assistant.utils import to_adk_content
...
class FarmwiseService:
    ...
    async def invoke(
        ...
        if memories:
            ...
            context.history = [
                genai.types.Content(
                    role="user" if m.role == "user" else "model", parts=[genai.types.Part.from_text(m.content)]
                )
                for m in memories
            ]
