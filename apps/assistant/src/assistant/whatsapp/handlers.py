...
from assistant.context import get_or_create_user
from assistant.schema import ActivityData, AudioResponse, CourseData, TextResponse, UserInput
from assistant.settings import settings
from assistant.storage import make_blob_public, upload_bytes_to_gcs
from assistant.whatsapp import commands
from assistant.whatsapp.activities import activities_section_list
from assistant.whatsapp.flows.courses.courses import get_courses_section_list
from assistant.whatsapp.flows.edit_profile.handlers import launch_edit_profile_flow
from assistant.whatsapp.responses import send_audio_reply, send_responses, send_text_reply
...
async def location_handler(_: WhatsApp, msg: types.Message):
    ...
    response_events = assistant.invoke(context, user_input)
...
async def on_activity_callback_selection(_: WhatsApp, sel: types.CallbackSelection[ActivityData]):
    ...
    response_events = assistant.invoke(context, user_input, agent_name=sel.data.agent)
...
async def image_handler(_: WhatsApp, msg: types.Message):
    ...
    logger.info(f"uploading image to {settings.FARMBETTER_STORAGE} blob_name: {blob_name}")
    bucket_name = settings.FARMBETTER_STORAGE.replace("gs://", "")
