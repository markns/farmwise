...
# ————————————————————————————————————————
# Crop Cycle, Stage & Event Schemas
# ————————————————————————————————————————


class CropCycleStageBase(FarmbaseBase):
    """Base model for crop cycle stages."""

    order: int = Field(description="Stage order within the cycle")
    name: str = Field(description="Stage name")
    duration: int = Field(description="Stage duration in days")


class CropCycleStageRead(CropCycleStageBase):
    """Model for reading crop cycle stage data."""
...
class CropCycleStageCreate(CropCycleStageBase):
    """Model for creating a new crop cycle stage."""

    pass


class CropCycleEventBase(FarmbaseBase):
    """Base model for crop cycle events."""

    event_identifier: str = Field(description="Event identifier (foreign key)")
    start_day: int = Field(description="Event start day from planting")
    end_day: int = Field(description="Event end day from planting")
    original_event_id: Optional[int] = Field(default=None, description="Original event ID from JSON")


class CropCycleEventRead(CropCycleEventBase):
    """Model for reading crop cycle event data."""

    id: PrimaryKey
    crop_cycle_id: int
    created_at: datetime
    updated_at: datetime
    event: Optional[EventRead] = Field(default=None, description="Associated event details")


class CropCycleEventCreate(CropCycleEventBase):
    """Model for creating a new crop cycle event."""
...
class CropCycleBase(FarmbaseBase):
    ...
    koppen_climate_classification: str = Field(description="Köppen climate classification", max_length=10)
...
class CropCycleRead(CropCycleBase):
    ...
    created_at: datetime
    updated_at: datetime
    stages: List[CropCycleStageRead] = Field(default_factory=list)
    events: List[CropCycleEventRead] = Field(default_factory=list)
    crop: Optional[CropRead] = Field(default=None, description="Associated crop details")
...
class CropCycleUpdate(FarmbaseBase):
    """Model for updating an existing crop cycle."""

    crop_id: Optional[str] = Field(default=None)
    koppen_climate_classification: Optional[str] = Field(default=None, max_length=10)
...
class SearchFilters(FarmbaseBase):
    ...
    koppen_climate_classification: Optional[str] = Field(
        default=None, description="Filter by Köppen climate classification"
    )
